...
const state = { type: "image", ratio: "1:1", session: null, signup: false, client: null };
...
async function refreshAccount(){
  ...
  try { const data=await api("me"); $("#creditPill").classList.remove("hidden"); $("#upgradeButton").classList.toggle("hidden",data.owner); $("#creditPill").textContent=data.owner?"∞ Owner":`${data.credits} credits`; $("#authButton").textContent="Log out"; }
...
async function loadLibrary(){
  ...
  try { const {items}=await api("library"); grid.innerHTML=""; $("#libraryEmpty").classList.toggle("hidden",items.length>0); items.forEach(item=>{ const card=document.createElement("article"); card.className="media-card"; const media=item.kind==="video"?document.createElement("video"):document.createElement("img"); media.src=item.output_url; if(item.kind==="video"){media.controls=true;media.playsInline=true;} else media.alt=item.prompt; const cap=document.createElement("div"); cap.innerHTML=`<span>${item.kind}</span><p></p><a target="_blank" rel="noopener">Open ↗</a>`; cap.querySelector("p").textContent=item.prompt; cap.querySelector("a").href=item.output_url; card.append(media,cap); grid.append(card); }); }
...
async function generate(){
  ...
  try { const {generation}=await api("generate",{method:"POST",body:JSON.stringify({type:state.type,prompt,aspectRatio:state.ratio})}); const media=generation.type==="video"?`<video src="${generation.outputUrl}" controls autoplay muted playsinline></video>`:`<img src="${generation.outputUrl}" alt="Generated artwork">`; box.className="result"; box.innerHTML=`${media}<div class="result-meta"><div><span>JUST CREATED</span><p></p></div><a class="button secondary" href="${generation.outputUrl}" target="_blank" rel="noopener">Open original ↗</a></div>`; box.querySelector(".result-meta p").textContent=prompt; await refreshAccount(); }
...
$("#upgradeButton").addEventListener("click",async()=>{ try { const {url}=await api("create-checkout",{method:"POST",body:"{}"}); location.href=url; } catch(error){ toast(error.message,true); } });
